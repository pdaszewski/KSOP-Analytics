  ...
  Begin
   ...
   czy_polaczono_db := False;
    ...
    if (czy_polaczono_db=True) then
     Begin
      //Jeœli wszystko dobrze, to uruchamiam przeszukiwanie
      StatusBar1.Panels[1].Text:='Poprawnie po³¹czono z baz¹ danych KSOP Analytics';
      PageControl1.Visible:=True;
      Szukaj_karty;
      Wypelnij_liste_izotopow;
      btn_nowaClick(Self);
     end
    else ShowMessage('Nie mo¿na pod³¹czyæ siê do serwera bazy danych aplikacji!');
  End
 else
  Begin
...
begin
 ...
 btn_zapisz_karte.Caption:='dodaj kartê do bazy';
...
begin
 ...
 if btn_zapisz_karte.Caption='dodaj kartê do bazy' then
  Begin
   if Sprawdz_czy_karta_juz_istnieje(numer)=False then
    ...
    End
   else ShowMessage('B£¥D!!!'+#13+'Karta o numerze: '+numer+' ju¿ istnieje w bazie danych!');
  End
 else
  Begin
   //to znaczy, ¿e edytujemy ju¿ istniej¹c¹ kartê
...
begin
 ...
 if (rodzaj='Beczka barytowa') and (edt_objetosc.Text='0') then
  ...
  End;
 if (rodzaj='Bêben 200') and (edt_objetosc.Text='0') then
  Begin
   edt_objetosc.Text:='0,2';
   cmb_kategoria.ItemIndex:=cmb_kategoria.Items.IndexOf('niskoaktywne');
  End;
...
begin
 ...
 edt_zrodel.Text:='0';
 edt_symbol.Clear;
 cmb_kategoria.ItemIndex:=cmb_kategoria.Items.IndexOf('œrednioaktywne');
 cmb_podkategoria.ItemIndex:=cmb_podkategoria.Items.IndexOf('krótko¿yciowe');
end;
...
begin
 ...
 +#13+'Program opracowany przez firmê FX Systems Piotr Daszewski'
 +#13+'Ró¿an ''2016 - ''2017'
 +#13+'Wszystkie prawa zastrze¿one!')
